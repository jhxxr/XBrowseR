...
const BROWSER_DATA_DIR_NAME = 'browser-kernels';
...
const migratedLegacyOfficialRoots = new Set();
...
function getManagedBrowserDataDir(baseDir) {
    return path.join(baseDir, 'data', BROWSER_DATA_DIR_NAME);
}

function getOfficialBrowserRoot(baseDir) {
    return path.join(getManagedBrowserDataDir(baseDir), OFFICIAL_BROWSER_VARIANTS_DIR);
}

function getLegacyOfficialBrowserRoot(baseDir) {
...
function isRetryableDownloadError(error) {
    ...
    return /timed out|timeout|econnreset|eai_again|enotfound|socket disconnected|network socket disconnected|aborted|econnrefused|ehostunreach|enetunreach|epipe|econnaborted/i.test(message);
}
...
async function downloadFile(url, outputPath, { onProgress } = {}) {
    ...
    await new Promise((resolve, reject) => {
        ...
        response.on('error', finalizeError);
        response.on('aborted', () => finalizeError(new Error('browser download aborted before completion')));
        ...
        fileStream.on('error', finalizeError);
    });
    reportProgress(onProgress, {
        phase: 'download',
        percent: 100,
...
async function downloadFileWithRetries(url, outputPath, { onProgress, maxAttempts = 3 } = {}) {
    let lastError = null;
        ...
        try {
            if (attempt > 1) {
                reportProgress(onProgress, {
                    phase: 'prepare',
                    percent: 2,
                    message: `retrying browser download (${attempt}/${maxAttempts})`
                });
            }
            await downloadFile(url, outputPath, { onProgress });
            const archiveStat = await fs.stat(outputPath).catch(() => null);
            if (!archiveStat || archiveStat.size <= 0) {
                throw new Error(`Downloaded browser archive missing or empty: ${outputPath}`);
            }
            return outputPath;
        } catch (error) {
            lastError = error;
    ...
    throw lastError || new Error('browser download failed');
}
...
function isInstalledBrowserRecordValid(rootDir, fallbackRevision = '') {
    const metadata = readJsonSyncSafe(path.join(rootDir, VERSION_FILE_NAME)) || {};
    const record = normalizeInstalledBrowserRecord(rootDir, metadata, fallbackRevision);
    return !!(record.id && record.executablePath && fs.existsSync(record.executablePath));
}

function migrateLegacyOfficialBrowsers(baseDir) {
    const resolvedBaseDir = normalizeString(baseDir) ? path.resolve(baseDir) : '';
    if (!resolvedBaseDir || migratedLegacyOfficialRoots.has(resolvedBaseDir)) {
        return [];
    }
    migratedLegacyOfficialRoots.add(resolvedBaseDir);

    const legacyRoot = getLegacyOfficialBrowserRoot(resolvedBaseDir);
    const targetRoot = getOfficialBrowserRoot(resolvedBaseDir);
    if (!fs.existsSync(legacyRoot) || path.resolve(legacyRoot) === path.resolve(targetRoot)) {
        return [];
    }
    ...
    for (const entry of fs.readdirSync(legacyRoot, { withFileTypes: true })) {
        ...
        if (!isInstalledBrowserRecordValid(sourceDir, entry.name)) {
            continue;
        }

        const targetDir = path.join(targetRoot, entry.name);
        if (isInstalledBrowserRecordValid(targetDir, entry.name)) {
            continue;
        }
...
function listInstalledBrowsers(baseDir) {
    migrateLegacyOfficialBrowsers(baseDir);
...
async function installOfficialBrowserRevision(baseDir, revision, { force = false, onProgress } = {}) {
    ...
    const installRoot = getOfficialBrowserRoot(baseDir);
    const archiveRoot = path.join(getManagedBrowserDataDir(baseDir), '.downloads');
    const archivePath = path.join(archiveRoot, `${normalizedRevision}-${platformId || 'browser'}.zip`);
    ...
    try {
        await downloadFileWithRetries(downloadUrl, archivePath, {
            maxAttempts: 3,

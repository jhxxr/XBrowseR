...
function normalizeGeolocation(input = {}) {
    ...
    let permission = String(source.permission || '').trim() || (mode === 'block' ? 'block' : 'allow');
    if (!['auto', 'manual', 'block'].includes(mode)) {
        mode = hasCoords ? 'manual' : 'auto';
    }
    if (!['allow', 'prompt', 'block'].includes(permission)) {
        permission = mode === 'block' ? 'block' : 'allow';
    }
    if (mode === 'block') {
        permission = 'block';
        mode = hasCoords ? 'manual' : 'auto';
...
function normalizeMediaSettings(input = {}) {
    ...
    return {
        ...
        imageEnabled: source.imageEnabled !== false,
        videoEnabled: source.videoEnabled !== false
    };
}

function normalizeWindowSettings(input = {}, resolution = {}) {
    const source = input && typeof input === 'object' ? input : {};
    const width = Math.max(480, Number(source.width) || Number(resolution.width) || 1280);
    const height = Math.max(480, Number(source.height) || Number(resolution.height) || 900);
    const sizeMode = source.sizeMode === 'fullscreen' || source.fullscreen === true ? 'fullscreen' : 'custom';
    const position = [
        'top-left', 'top-center', 'top-right',
        'center-left', 'center', 'center-right',
        'bottom-left', 'bottom-center', 'bottom-right'
    ].includes(String(source.position || '').trim())
        ? String(source.position).trim()
        : 'top-left';
...
function normalizeStartupPreferences(input = {}) {
    const source = input && typeof input === 'object' ? input : {};
    return {
...
function createFingerprint(input = {}) {
    ...
    const normalizedGeolocation = normalizeGeolocation(input.geolocation || {});
    const normalizedMedia = normalizeMediaSettings(input.media || {});
    const normalizedWindow = normalizeWindowSettings(input.window || {}, resolution);
    const searchEngine = ['google', 'bing', 'duckduckgo', 'baidu'].includes(String(input.searchEngine || '').trim())
        ? String(input.searchEngine).trim()
        : 'google';
    const startupUrls = normalizeUrlList(input.startupUrls || input.extraStartupUrls || []);
    const preferences = normalizeStartupPreferences(input.preferences || {});
    const storagePreset = {
        cookies: normalizeStructuredText(input.storagePreset?.cookies ?? input.cookiesPreset ?? ''),
        localStorage: normalizeStructuredText(input.storagePreset?.localStorage ?? input.localStoragePreset ?? '')
    };
    ...
    return {
        ...
        uiLanguage: input.uiLanguage === '' || input.uiLanguage == null ? 'auto' : input.uiLanguage,

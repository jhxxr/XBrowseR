...
function ensureAgentSettings(settings = {}) {
    const providers = Array.isArray(settings.agent?.providers)
        ...
        : [];
    const toolTimeoutMs = Math.max(5000, Number(settings.agent?.toolTimeoutMs) || 20000);
    const maxExecutionSteps = Math.max(3, Math.min(50, Number(settings.agent?.maxExecutionSteps) || 10));

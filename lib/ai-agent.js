...
function getMaxExecutionSteps(agentSettings = {}) {
    const raw = Number(agentSettings?.maxExecutionSteps);
    return Math.max(3, Math.min(50, raw || 10));
}
...
function createManualHandoffError(message = 'manual handoff required') {
    const error = new Error(message);
    error.code = 'MANUAL_HANDOFF';
    return error;
}

function isManualHandoffError(error) {
    return error?.code === 'MANUAL_HANDOFF' || /manual handoff|required.*human|人工接管/.test(String(error?.message || ''));
}

async function detectManualHandoff(page) {
    ...
    try {
        const snapshot = await page.evaluate(() => {
            ...
            return {
                ...
                text: bodyText,
                hasPasswordInput: !!document.querySelector('input[type="password"], input[autocomplete="current-password"], input[autocomplete="new-password"]'),
                hasOtpInput: !!document.querySelector('input[autocomplete="one-time-code"], input[name*="otp" i], input[id*="otp" i], input[name*="code" i], input[id*="code" i]'),
                hasCaptchaWidget: !!document.querySelector('iframe[src*="captcha" i], iframe[src*="turnstile" i], .cf-turnstile, [data-sitekey], [class*="captcha" i], [id*="captcha" i]'),
                hasPaymentField: !!document.querySelector('input[autocomplete="cc-number"], input[autocomplete="cc-exp"], input[autocomplete="cc-csc"], [name*="cardnumber" i], [name*="payment" i], [id*="payment" i], [name*="cvv" i], [id*="cvv" i]')
            };
        });

        const combined = `${snapshot.url}\n${snapshot.title}\n${snapshot.text}`;
        const challengePattern = /(cloudflare|turnstile|captcha|verify you are human|checking your browser|attention required|security check|人机验证|验证您是真人|安全检查|机器人验证)/i;
        const loginPattern = /(sign in|sign-in|log in|login|登录|登錄|password|密码|passkey|two-factor|2fa|otp|验证码|verification code|身份验证)/i;
        const paymentPattern = /(checkout|payment|billing|pay now|credit card|card number|cvv|银行卡|结账|支付|付款|收银台)/i;

        if (snapshot.hasCaptchaWidget || challengePattern.test(combined)) {
            return {
                reason: 'challenge',
                message: '检测到验证或挑战页面，需要人工接管后再继续'
            };
        }

        if ((snapshot.hasPasswordInput || snapshot.hasOtpInput) && loginPattern.test(combined)) {
            return {
                reason: 'auth',
                message: '检测到登录或身份验证页面，需要人工接管后再继续'
            };
        }

        if (snapshot.hasPaymentField || paymentPattern.test(combined)) {
            return {
                reason: 'payment',
                message: '检测到支付或结算页面，需要人工接管后再继续'
            };
        }
    } catch (error) {
        if (!/Target closed|closed|Execution context was destroyed/i.test(String(error?.message || ''))) {
            return null;
        }
    }
...
function createAgentController(dependencies) {
    ...
    async function runSessionMessage(session, message) {
        ...
        const maxExecutionSteps = getMaxExecutionSteps(dependencies.getAgentSettings?.() || {});
        ...
        try {
            for (let step = 0; step < maxExecutionSteps; step += 1) {
                ...
                const handoffRequirement = await detectManualHandoff(session.page);
                if (handoffRequirement) {
                    throw createManualHandoffError(handoffRequirement.message);
                }
                ...
                if (assistantMessage.toolCalls.length) {
                    ...
                    for (const toolCall of assistantMessage.toolCalls) {
                        ...
                        session.pageUrl = session.page.url();
                        const postToolHandoff = await detectManualHandoff(session.page);
                        if (postToolHandoff) {
                            throw createManualHandoffError(postToolHandoff.message);
                        }
            ...
            throw new Error(`agent reached the maximum execution steps (${maxExecutionSteps})`);
        } catch (error) {
            const handoffRequired = isManualHandoffError(error);
            const stopped = !handoffRequired && (isStopError(error) || session.abortController?.signal?.aborted);
            const normalizedError = handoffRequired
                ? error
                : (stopped
            ...
            pushEvent(session, {
                kind: 'status',
                level: handoffRequired || stopped ? 'warn' : 'error',
                code: handoffRequired ? 'MANUAL_HANDOFF' : '',
        ...
        try {
            for (let attempt = 1; attempt <= totalAttempts; attempt += 1) {
                ...
                try {
                    ...
                    if (isManualHandoffError(error)) {
                        task.status = 'stopped';
                        task.error = '';
                        task.summary = error?.message || '需要人工接管';
                        return;
                    }

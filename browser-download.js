...
function getPackedVersionFilePath() {
    const packedDir = getPackedBrowserDir();
    return packedDir ? path.join(packedDir, VERSION_FILE_NAME) : '';
}
...
function getPackedExecutablePath() {
    const packedDir = getPackedBrowserDir();
    const relativePath = getExecutableRelativePath();
    return packedDir && relativePath ? path.join(packedDir, relativePath) : '';
}
...
function getExecutableCandidates(baseDir) {
    ...
    if (relativePath) {
        ...
        const packedDir = getPackedBrowserDir();
        if (packedDir) {
            candidates.push(path.join(packedDir, relativePath));
        }
    }
...
async function getInstalledBrowserVersion(baseDir) {
    const packedVersion = await readInstalledVersion(getPackedVersionFilePath());
    if (packedVersion) {
        return packedVersion;
    }
    return readInstalledVersion(getVersionFilePath(baseDir));
}
...
async function ensureBundledBrowser(baseDir, { force = false } = {}) {
    ...
    try {
        download = await resolveChromeDownload(platformId);
    } catch (error) {
        if (!force) {
            if (packedPath && await fs.pathExists(packedPath)) {
                return packedPath;
            }
